...
def create_runner_app(
    *,
    ...
    async def _persist_sdk_compaction(conv_id: str, evt: dict[str, Any]) -> None:
        """
        Persist a harness-emitted compaction summary as a durable
        ``compaction`` item so a resumed session recovers the boundary.

        Mirrors the proactive-compaction persist in
        :func:`_proactive_compact_if_needed`: a ``compaction`` item needs a
        real server-side ``last_item_id`` or it would poison the history
        cursor — so skip silently when none is known yet.

        :param conv_id: Session/conversation identifier.
        :param evt: The relayed ``response.compaction.completed`` event dict,
            carrying ``summary`` / ``summary_model`` / ``total_tokens``.
        :returns: None.
        """
        summary = evt.get("summary")
        if not isinstance(summary, str) or not summary:
            return
        last_item_id = _last_server_item_id.get(conv_id)
        if not last_item_id:
            _logger.warning(
                "Skipping claude-sdk compaction persist for %s: no server-side "
                "last_item_id available yet",
                conv_id,
            )
            return
        compaction_event = {
            ...
            "summary": summary,
            "last_item_id": last_item_id,
            "model": evt.get("summary_model"),
            "token_count": evt.get("total_tokens"),
        }
        try:
            await server_client.post(
                f"/v1/sessions/{conv_id}/events",
                json={"type": "compaction", "data": compaction_event},
                timeout=10.0,
            )
        except (httpx.HTTPError, RuntimeError):
            _logger.warning(
                "Failed to persist claude-sdk compaction item for %s",
                conv_id,
                exc_info=True,
            )
                        ...
                        while _drain and "\n\n" in _buf:
                            ...
                            for line in frame.splitlines():
                                ...
                                if _etype == "response.compaction.completed":
                                    ...
                                    await _persist_sdk_compaction(conv_id, evt)
